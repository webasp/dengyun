...
layui.define(['layer', 'form'], function (exports) {
    ...
    var tools = {
        face: '<span title="插入表情"><i class="layui-icon" event="face">&#xe650;</i><span event="face" class="tool-tip">表情</span></span>',
        image: '<span title="插入图片"><i class="layui-icon" event="image">&#xe64a;</i><span event="image" class="tool-tip">图片</span></span>',
        link: '<span event="link" title="插入链接"><i class="layui-icon" event="link">&#xe64d;</i><span class="tool-tip">链接</span></span>',
        code: '<span event="code" title="插入代码" class="divide"><i class="layui-icon" event="code">&#xe64e;</i><span class="tool-tip">代码</span></span>',
        full_screen: '<span event="full_screen" title="全屏"><i class="layui-icon" event="full_screen">&#xe638;</i><span class="tool-tip">全屏</span></span>',
        subfield: '<span event="subfield" title="分栏预览" class="hide"><i class="layui-icon" event="subfield">&#xe649;</i><span class="tool-tip">分栏</span></span>',
        preview: '<span event="preview" title="全屏预览"><i class="layui-icon" event="preview">&#xe615;</i><span class="tool-tip">预览</span></span>'
    };
    ...
    markdown.prototype.build = function (id, settings) {
        ...
        var editor = $([
            ...
            '  <div class="tools">' + _tools + '</div>',
            '  <textarea placeholder="请输入内容" class="layui-textarea" ' + _required + ' name="markdown_content" v-model="markdown_content"></textarea>',
            '</div>'].join('')
        );
        ...
        // 图片
        dom.find("i[event='image'],span[event='image']").click(function () {
    ...
    var code = function(obj, callback) {
        ...
        return _layer.open({
            ...
            "    <button type='button' name='yes' class='layui-btn'><i class='layui-icon'>&#xe7bd;</i>&ensp; 确&ensp;定 </button>" +
    ...
    var image = function (obj, image_upload_action, callback) {
        ...
        image.hide = image.hide || function (e) {
            if ($(e.target).attr('event') !== 'image') {
                _layer.close(image.index);
            }
        };


        var _idx = image.index = _layer.tips((function () {
            var content = [];
            ...
            success: function (layero, index) {
                ...
                $(document).off('click', image.hide).on('click', image.hide);
            }
        });
        if ($(window).width() < 450) {
        ...
        var _idx = _layer.open({
            ...
            "      <i class='layui-icon' style='position: relative; top: -3px; font-size: 18px;'>&#xe61f;</i> <span class='tool-tip'>上传</span></button>" +
